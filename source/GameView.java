...
public class GameView extends AbstractView<GameModel> implements Runnable {
	...
	public GameView(PropertyChangeListener listener, String playerName, int time,
		...
		shouldScroll = true;
		buttonGraphic = new IconComponent("images/btn.jpg");
		...
		timerNotificationSound = java.applet.Applet.newAudioClip(
				this.getClass().getResource("sounds/longBeep.wav"));
		timerStartSound = java.applet.Applet.newAudioClip(
				this.getClass().getResource("sounds/shortBeep.wav"));
		checkSound = java.applet.Applet.newAudioClip(
				this.getClass().getResource("sounds/check.wav"));
		callSound = java.applet.Applet.newAudioClip(
				this.getClass().getResource("sounds/call.wav"));
		betSound = java.applet.Applet.newAudioClip(
				this.getClass().getResource("sounds/bet.wav"));
		raiseSound = java.applet.Applet.newAudioClip(
				this.getClass().getResource("sounds/raise.wav"));
		foldSound = java.applet.Applet.newAudioClip(
				this.getClass().getResource("sounds/fold.wav"));
	...
	public void run() {
    	...
    	displayFrame.addComponentListener(new ComponentAdapter() {
    		public void componentResized(ComponentEvent evt) {
		        ...
		        if (lastWidth != currentWidth) {
					...
					/*updatePanel(boxPanel, ratio);
					updatePanel(raiseSizePanel, ratio);
					updatePanel(bgPanel, ratio);
					updatePanel(buttonPanel, ratio);
					updatePanel(potLabelPanel, ratio);
					updatePanel(boardPanelWrapper, ratio);
					updatePanel(actionLabelPanel, ratio);
					updatePanel(chatPanel, ratio);*/
		...
		/*displayFrame.addComponentListener(new ComponentAdapter() {
			public void componentResized(ComponentEvent evt) {
				...
				double ratio = 1;
				if (oldWidth != 0) {
					ratio = 1.0 * displayFrame.getWidth() / oldWidth;
				}
				oldWidth = displayFrame.getWidth();
				fixBounds(actionLabel, ratio);
				bg.rescale(ratio);
				for (PlayerBoxComponent pbc : boxes) {
					pbc.rescale(ratio);
				}
				boardPanel.rescale(ratio);
				fixBounds(buttonGraphic, ratio);
				for (JLabel jl : raiseSizes) {
					fixBounds(jl, ratio);
				}
				fixBounds(potLabel, ratio);
				adjustCoords(ratio, coords);
				adjustCoords(ratio, buttonCoords);
				adjustCoords(ratio, raiseSizeCoords);
				fixBounds(chatScrollPane, ratio);
			}
		});*/
