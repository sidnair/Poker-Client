...
public class EvalHandEvaluator implements Serializable {
   ...
   public static void main(String args[]){
       EvalHand hand1 = new EvalHand("Kc 8c 9c Tc Jc Qc Ac");
       EvalHand hand2 = new EvalHand("Kc 8c 9c Tc Jc Qc Ad");

       EvalHandEvaluator handEval = new EvalHandEvaluator();
    ...
    */
   public static String nameHand(EvalHand h) {
    ...
    */
   public int[][] getRanks(EvalHand board) {
      EvalHand myhand = new EvalHand(board);
      int[][] rc = new int[52][52];
      int i, j, v, n1, n2;
      EvalDeck d = new EvalDeck();
      d.extractHand(board);

      // tabulate ranks
      for (i = d.getTopCardIndex(); i < EvalDeck.NUM_CARDS; i++) {
         myhand.addCard(d.getCard(i));
         n1 = d.getCard(i).getIndex();
         for (j = i + 1; j < EvalDeck.NUM_CARDS; j++) {
   ...
   private static final int ID_GROUP_SIZE = (EvalCard.NUM_RANKS * EvalCard.NUM_RANKS
         * EvalCard.NUM_RANKS * EvalCard.NUM_RANKS * EvalCard.NUM_RANKS);
   ...
   private final static byte ID_ExistsStraightFlush(EvalHand h, byte major_suit) {
      ...
      for (int i = 0; i < h.size(); i++) {
         ...
         if (EvalCard.getSuit(cind) == major_suit) {
            present[EvalCard.getRank(cind)] = true;
         }
      }
      int straight = present[EvalCard.ACE] ? 1 : 0;
      byte high = 0;
      for (int i = 0; i < EvalCard.NUM_RANKS; i++) {
         if (present[i]) {
   ...
   private final static int ID_KickerValue(byte[] paired, int kickers,
      ...
      while (kickers != 0) {
         while (paired[i] == 0 || i == not_allowed[0] || i == not_allowed[1])
            i--;
         kickers--;
         value += pow(EvalCard.NUM_RANKS, kickers) * i;
         i--;
      }
      return value;
   }
   ...
   public final static int rankHand(EvalHand h) {
      ...
      byte[] paired = new byte[EvalCard.NUM_RANKS];       // array to track paired carsd
      ...
      byte straight_size;
      // flush
      byte[] suit_size = new byte[EvalCard.NUM_SUITS];
      byte major_suit = 0;
      ...
      for (r = 0; r < h.size(); r++) {
         ...
         rank = (byte) EvalCard.getRank(cind);
         suit = (byte) EvalCard.getSuit(cind);
      ...
      straight_size = (byte) (paired[EvalCard.ACE] != 0 ? 1 : 0);

      for (int i = 0; i < (POKER_HAND + 1); i++) {
         pair_rank[i][0] = (byte) EvalCard.NUM_RANKS;
         pair_rank[i][1] = (byte) EvalCard.NUM_RANKS;
      }

      // check for straight and pair data
      // Start at the Deuce.  straight_size = 1 if we have an ace.
      for (r = 0; r < EvalCard.NUM_RANKS; r++) {
         // check straight
         if (paired[r] != 0) {
      ...
      if (group_size[4] != 0) {
         ...
         ident += pair_rank[4][0] * EvalCard.NUM_RANKS;
         pair_rank[4][1] = (byte) EvalCard.NUM_RANKS; // just in case 2 sets quads
         ident += ID_KickerValue(paired, 1, pair_rank[4]);
      } else if (group_size[3] >= 2) {
         ident = FULLHOUSE * ID_GROUP_SIZE;
         ident += pair_rank[3][0] * EvalCard.NUM_RANKS;
         ident += pair_rank[3][1];
         ...
         ident += pair_rank[3][0] * EvalCard.NUM_RANKS * EvalCard.NUM_RANKS;
         ident += ID_KickerValue(paired, max_hand - 3, pair_rank[3]);
      } else if (group_size[2] >= 2) {    // TWO PAIR
         ident = TWOPAIR * ID_GROUP_SIZE;
         ident += pair_rank[2][0] * EvalCard.NUM_RANKS * EvalCard.NUM_RANKS;
         ident += pair_rank[2][1] * EvalCard.NUM_RANKS;
